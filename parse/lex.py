...
@dataclass
class InlineCommentToken(Token):
    body: str
...
class Parser:
    ...
    def parse(self) -> None:
        ...
        try:
            while self.stuff_left():
                s.append(self.get_statement())
        except:
            # for x in s:
            #     self.dump_statement_tree(x)
    ...
    def _get_statement(self) -> Statement:
        ...
        # For
        # HACK
        elif self.src[self.i - 1] != "." and self.accept("for"):
            for_parameters = self.get_statement()
            assert isinstance(for_parameters, ExpressionStatement)
            self.eat_whitespace()
            children = []
            if self.accept("{"):
                while not self.accept("}"):
                ...
                children.append(self.get_statement())
            return ForStatement(for_parameters, children)
        # Class
        ...
        # If
            ...
            if self.accept("{"):
                ...
                while True:
                    c = self.get_statement()
                    children.append(c)
                    print(":: SIMPLEIF :", c)
                    if self.src[self.i] == "\n":
                        break
                    if isinstance(c, SemicolonStatement):
                        break
        ...
        # RegEx literal
        # HACK
        elif self.src[self.i] == "/" and self.src[self.i - 1] in ["(", "\n", "\t", " "]:
            self.i += 1
            out = ""
            while True:
                char = self.src[self.i]
